...
<template>
  ...
  <section class="panel search-results">
    <p v-if="state.story_cluster_id.length" class="muted story-filter-banner">Filtered to one story <button type="button" class="secondary" @click="navigate({ ...state, story_cluster_id: [] })">Clear story filter</button></p>
    ...
    <article v-for="result in results" :key="result.article_id" class="search-result"><button class="result-open" @click="openArticle(result.article_id)"><strong>{{ result.title }}</strong><span>{{ new Date(result.effective_date).toLocaleString() }} · {{ result.distinct_source_count }} sources</span><p v-if="result.highlights.length"><template v-for="(segment, index) in result.highlights" :key="index"><mark v-if="segment.marked">{{ segment.text }}</mark><template v-else>{{ segment.text }}</template></template></p><p v-else-if="result.summary">{{ result.summary }}</p></button><div class="result-filters"><button v-for="source in result.source_refs" :key="source.id" type="button" class="annotation-link" :aria-label="`Filter by source ${source.name}`" @click="crossFilter('source_id', source.id)">{{ source.name }}</button><button v-for="country in sourceCountries(result.source_refs)" :key="`source-${country}`" type="button" class="annotation-link" :aria-label="`Filter by source country ${country}`" @click="crossFilter('source_country', country)">{{ country }}</button><button v-if="result.story_country" type="button" class="annotation-link" :aria-label="`Filter by story country ${result.story_country}`" @click="crossFilter('story_country', result.story_country)">Story: {{ result.story_country }}</button><RouterLink v-if="result.story_cluster && otherSources(result) > 0" class="annotation-link" :to="{ path: `/clusters/${result.story_cluster.id}`, query: { from: route.fullPath } }">Also reported by {{ otherSources(result) }} other source{{ otherSources(result) === 1 ? '' : 's' }}</RouterLink></div></article>

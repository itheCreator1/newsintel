...
export const api = {
  ...
  facets: (filters: Filters, limit?: number) => {
    const params = filterParams(filters)
    if (limit) params.set('limit', String(limit))
    return request<SearchFacets>(`/search/facets?${params}`)
  },
  ...
  monitors: (order: 'name' | 'activity', cursor?: string, limit?: number) => request<MonitorPage>(`/monitors?${new URLSearchParams({ order, ...(cursor ? { cursor } : {}), ...(limit ? { limit: String(limit) } : {}) })}`),

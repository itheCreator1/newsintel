...
run_id="$$-$(date +%s)"
project="newsintel-phase15c-$run_id"
...
# 15C adds no migration; the head must stay single.
...
# Analytics on Elasticsearch (SQL parity, discovery vs publication, scope), plus the shared builders' other callers.
(cd backend && NEWSINTEL_RUN_POSTGRES_TESTS=1 uv run --frozen python -m pytest -q \
  tests/test_analytics_routes.py tests/test_analytics_elasticsearch.py tests/test_analytics_timeline.py \
  tests/test_search_aggregations.py tests/test_search_facets.py tests/test_search_facets_elasticsearch.py \
  tests/test_search_timeline.py tests/test_search_results.py tests/test_search_pagination_elasticsearch.py \
  tests/test_monitor_evaluation_elasticsearch.py tests/test_monitor_results_elasticsearch.py) > "$artifacts/pytest.log"
tail -3 "$artifacts/pytest.log"
(cd backend && uv run --frozen ruff check app && uv run --frozen python -m mypy app)

# The analytics routes gain the search criteria and scope, so the committed contract must match a regeneration.
...
# Regression only; no browser spec asserts the Overview panels (Vitest covers their states).
...
echo "Phase 15C acceptance passed: project=$project artifacts=$artifacts"

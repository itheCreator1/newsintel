...
run_id="$$-$(date +%s)"
project="newsintel-phase15a-$run_id"
...
# 15A adds no migration; the head must stay single.
...
# Real PIT paging (last page closes its snapshot), plus the search consumers that share build_query.
(cd backend && NEWSINTEL_RUN_POSTGRES_TESTS=1 uv run --frozen python -m pytest -q \
  tests/test_search_results.py tests/test_search_timeline.py tests/test_graph_entities.py tests/test_graph_edge_evidence.py \
  tests/test_search_pagination_elasticsearch.py tests/test_graph_edge_evidence_postgres.py \
  tests/test_monitor_evaluation_elasticsearch.py tests/test_monitor_results_elasticsearch.py tests/test_monitor_changes_elasticsearch.py) > "$artifacts/pytest.log"
tail -3 "$artifacts/pytest.log"
(cd backend && uv run --frozen ruff check app && uv run --frozen python -m mypy app)

# Graph evidence gained cluster_count_estimated, so the committed contract must match a regeneration.
...
# Browser coverage lives in the maintained groups: search paging and the graph evidence wording.
...
echo "Phase 15A acceptance passed: project=$project artifacts=$artifacts"

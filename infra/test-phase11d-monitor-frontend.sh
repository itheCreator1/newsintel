...
run_id="$$-$(date +%s)"
project="newsintel-phase11d-$run_id"
...
# The one backend change: monitor models must resolve their foreign keys without the API's imports.
(cd backend && UV_CACHE_DIR="$root/backend/.uv-cache" uv run --frozen python -m pytest -q tests/test_monitors.py tests/test_monitor_api.py tests/test_monitor_evaluation.py)
...
# The frontend suite is flaky-sensitive (Phase 11D removed an unmounted-component race), so run it repeatedly.
for attempt in 1 2 3; do (cd frontend && npm test) || { echo "npm test failed on attempt $attempt" >&2; exit 1; }; done
(cd frontend && npm run typecheck)
(cd frontend && npm run build)
# Phase 11D changes no API: the checked-in contract must still match the application.
...
# 11D adds no schema: the head must still be 0010.
$compose run --rm api alembic heads | tr -d '\r' | grep -q '^0010 (head)' || { echo "Phase 11D must not add a migration" >&2; exit 1; }
...
echo "Phase 11D acceptance passed: project=$project artifacts=$artifacts"

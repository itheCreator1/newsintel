...
run_id="$$-$(date +%s)"
project="newsintel-phase13a-$run_id"
...
# 13A adds no migration: the schema stays at 0012 (the EXPLAIN test decides whether one is ever needed).
migrate upgrade head
[ "$(tests_psql "select version_num from alembic_version")" = 0012 ] || { echo "Phase 13A changed the migration head" >&2; exit 1; }

host_test_database="postgresql+asyncpg://newsintel:newsintel@127.0.0.1:$NEWSINTEL_TEST_POSTGRES_PORT/newsintel_tests"
# The source API is PostgreSQL-only, so point Elasticsearch at an unreachable port to prove it.
...
(cd backend && UV_CACHE_DIR="$root/backend/.uv-cache" uv run --frozen python -m ruff check app tests/test_source_api.py tests/test_source_dossier_postgres.py tests/event_fixtures.py migrations)
(cd backend && UV_CACHE_DIR="$root/backend/.uv-cache" uv run --frozen python -m mypy app)

# The source routes change the API contract, so regenerate it and the frontend types, then prove the
...
$compose run --rm api alembic heads | tr -d '\r' | grep -q '^0012 (head)' || { echo "Phase 13A must not add a migration" >&2; exit 1; }
# `relationships seed` signs in as phase7; the entity spec uses phase10b, the event spec phase12d, the source spec phase13a.
...
# The event and entity specs prove the new source links and the shared pages still work.
(cd frontend && npm run e2e -- --grep "event workflow")
(cd frontend && npm run e2e -- --grep "entity dossier workflow")

echo "Phase 13A acceptance passed: project=$project artifacts=$artifacts"

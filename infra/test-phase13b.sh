...
run_id="$$-$(date +%s)"
project="newsintel-phase13b-$run_id"
...
# 13B adds no migration: the schema stays at 0012 (a candidate country-annotation index did not change the
# plan on a 60k-article measurement, so none was added; see the roadmap).
migrate upgrade head
[ "$(tests_psql "select version_num from alembic_version")" = 0012 ] || { echo "Phase 13B changed the migration head" >&2; exit 1; }
...
$compose run --rm api alembic heads | tr -d '\r' | grep -q '^0012 (head)' || { echo "Phase 13B must not add a migration" >&2; exit 1; }
# `relationships seed` signs in as phase7; the entity spec uses phase10b, the event spec phase12d, the source spec phase13a, the compare spec phase13b.
...
# The source, event and entity specs prove the new Compare links and the shared pages still work.
...
echo "Phase 13B acceptance passed: project=$project artifacts=$artifacts"

...
cleanup() {
  rm -f "$dump" "$expected_file" "$actual_file"
  pg dropdb -U "$user" --if-exists "$copy_db" >/dev/null 2>&1 || true
}
...
counts "$source_db" > "$expected_file"
counts "$copy_db" > "$actual_file"
[ -s "$expected_file" ] || { echo "No tables in $source_db" >&2; exit 1; }
if ! cmp -s "$expected_file" "$actual_file"; then
  echo "Restored row counts differ from $source_db:" >&2
  diff "$expected_file" "$actual_file" >&2 || true
  exit 1
fi
echo "Restored $(wc -l < "$expected_file") tables, $(awk '{s += $2} END {print s}' "$expected_file") rows"

...
run_id="$$-$(date +%s)"
project="newsintel-phase15b-$run_id"
...
# 15B adds no migration; the head must stay single.
...
# Bounded /search/facets, plus the search/graph/monitor consumers that share build_query.
(cd backend && NEWSINTEL_RUN_POSTGRES_TESTS=1 uv run --frozen python -m pytest -q \
  tests/test_search_facets.py tests/test_search_facets_elasticsearch.py tests/test_search_results.py tests/test_search_timeline.py \
  tests/test_search_pagination_elasticsearch.py tests/test_graph_entities.py tests/test_graph_edge_evidence.py \
...
# /search/facets adds SearchFacets, so the committed contract must match a regeneration.
...
# Regression only; facet browser assertions arrive with the 15D panels.
...
echo "Phase 15B acceptance passed: project=$project artifacts=$artifacts"

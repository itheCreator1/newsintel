...
run_id="$$-$(date +%s)"
project="newsintel-phase15f-$run_id"
...
# 15F adds no migration; the head must stay single.
...
# The V3 mapping check (every body builder and route) and, on a real V3 index, the modules that
# create one: the builders it covers must still run there.
(cd backend && NEWSINTEL_RUN_POSTGRES_TESTS=1 uv run --frozen python -m pytest -q \
  tests/test_index_schema.py tests/test_search_facets_elasticsearch.py \
  tests/test_search_pagination_elasticsearch.py tests/test_geo_elasticsearch.py \
  tests/test_analytics_elasticsearch.py tests/test_monitor_results_elasticsearch.py \
  tests/test_monitor_changes_elasticsearch.py) > "$artifacts/pytest.log"
tail -3 "$artifacts/pytest.log"
(cd backend && uv run --frozen ruff check app && uv run --frozen python -m mypy app)

# 15F changes no API, so the committed contract must still match a regeneration.
...
echo "Phase 15F acceptance passed: project=$project artifacts=$artifacts"

...
- **Domain-driven backend**, not a god-object API: `feeds`, `articles`, `ingestion`, `extraction`, `nlp`, `search`, `clustering`, `analytics`, `entities`, `graph`, `investigations`, `monitors`, `events` (storage only so far), and `jobs` are separate modules under `backend/app`, each owning its models, service layer, and routes.
...
- **A dark glassmorphism UI** across all eleven routes (overview, search, articles, sources, clusters, entities, graph, jobs, saved searches, watchlist, settings) built with Next.js App Router, TanStack Query, and Apache ECharts.

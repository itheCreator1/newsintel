...
- **Domain-driven backend**, not a god-object API: `feeds`, `articles`, `ingestion`, `extraction`, `nlp`, `search`, `clustering`, `analytics`, `entities`, `graph`, `investigations`, `monitors`, `events`, and `jobs` are separate modules under `backend/app`, each owning its models, service layer, and routes.

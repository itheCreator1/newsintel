...
- **Events (12A storage, 12B engine, 12C API):** `backend/app/events` holds `Event`, `EventCluster` and `EventEntity` (tables `events`, `event_clusters`, `event_entities`, migration `0011`) plus repository primitives. `app/events/engine.py` is the replaceable v1 association engine (`EventAssociator` protocol, `RuleEventAssociator`, version `rule-1`): the scheduler sends the `associate_events` Dramatiq actor (`app.jobs.events`, queue `events`, worker with clustering) every 30 s, which decides each new or changed story cluster (`story_clusters.updated_at` newer than `event_clusters.cluster_updated_at`) by time, entity, headline and story-country signals and refreshes the event's cached fields. `app/events/{schemas,queries,routes}.py` expose it read-only under `/api/v1/events` (list, detail, clusters, articles, timeline; all behind `current_session`); counts and headlines are aggregated per page, never stored; no UI yet. An event carries an `algorithm_version`, a `status` (`active`/`closed`/`superseded`), a cached time span and `primary_country`; clusters and entities attach through normalized association rows.

...
- Current phase: Phase 11E — What Changed (branch `phase/11e-monitor-changes`)
- Current status: `COMPLETE`
- Next action: review and integrate Phase 11E into local `main` on the user's say-so (nothing is committed or pushed yet). Phase 12A (event domain/schema) may then begin from `main`.
- Deferred work: Every phase after 11E remains `NOT STARTED` until the preceding phase meets its acceptance criteria. Deferred inside 11D/11E: entity/source/country/cluster "Watch" buttons (the API supports those kinds), editing a monitor's criteria in the UI, an unseen badge in the navigation, persisted per-window change snapshots (would make story growth fully reproducible), and a lookback limit or seen-set table for the "never matched before" check.
- Verification state: Phases 10A (`2841cdc`), 10B (`0fd4126`), 10C (`dacd349`), 11A (`957b8aa`), 11B (`0f39ae0`), 11C (`74073cb`) and 11D (`5c0e22c`) are merged into local `main` (unpushed). Phase 11E `infra/test-phase11e.sh` (monitor backend tests, Ruff, mypy, Vitest 126 tests three times in a row, typecheck, static build, OpenAPI drift check, Alembic head still `0010`, the real Elasticsearch/PostgreSQL change-summary tests, `rebuild-search`, and the Playwright monitor workflow against the Compose stack) passed on 2026-09-20.

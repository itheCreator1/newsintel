...
- Current phase: Phase 11D — Monitor frontend (branch `phase/11d-monitor-frontend`)
- Current status: `COMPLETE`
- Next action: Phase 11E (What Changed) may begin from local `main` (11D is merged as `5c0e22c`; nothing is pushed). 11E extends the monitor detail (`frontend/src/components/MonitorDetail.tsx`) with deterministic change summaries over the same `(viewed_through, evaluated_through]` window the counters and results use.
- Deferred work: Every phase after 11D remains `NOT STARTED` until the preceding phase meets its acceptance criteria. Deferred inside 11D: entity/source/country/cluster "Watch" buttons (the API supports those kinds), editing a monitor's criteria in the UI, and an unseen badge in the navigation.
- Verification state: Phases 10A (`2841cdc`), 10B (`0fd4126`), 10C (`dacd349`), 11A (`957b8aa`), 11B (`0f39ae0`), 11C (`74073cb`) and 11D (`5c0e22c`) are merged into local `main` (unpushed). Phase 11D `infra/test-phase11d.sh` (monitor backend unit tests, Ruff, mypy, Alembic head still `0010`, Vitest 119 tests three times in a row, typecheck, static build, OpenAPI drift check, `rebuild-search`, and the Playwright monitor workflow against the Compose stack) passed on 2026-09-20.

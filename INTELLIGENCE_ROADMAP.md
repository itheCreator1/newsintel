...
- Current phase: Phase 11C — Monitor API/backend (branch `phase/11c-monitor-api`)
- Current status: `COMPLETE`
- Next action: Review and integrate `phase/11c-monitor-api`; Phase 11D must begin from the integration branch after that. 11D consumes the regenerated types (`Monitor*` schemas); the viewed operation takes `through` = the `window_end` of the results it showed (or `evaluated_through`), never a blind "mark all".
- Deferred work: Every phase after 11C remains `NOT STARTED` until the preceding phase meets its acceptance criteria.
- Verification state: Phases 10A (`2841cdc`), 10B (`0fd4126`), 10C (`dacd349`), 11A (`957b8aa`) and 11B (`0f39ae0`) are merged into local `main` (unpushed). Phase 11C `infra/test-phase11c.sh` (93 backend tests against real PostgreSQL and Elasticsearch, head still `0010`, Ruff, mypy, Vitest 93, typecheck, OpenAPI/type drift check) passed on 2026-09-20.

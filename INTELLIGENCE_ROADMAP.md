...
- **Status:** `COMPLETE` (on `phase/13b-compare-mode`, uncommitted)
...
| Timing is descriptive: first / median and p90 minutes behind, over stories with `source_count >= 2` | "First" is the earliest article by `(effective date, article id)`, so ties are deterministic. Single-source stories are excluded because there is nothing to be behind. There is no rank, score or quality label. |
...
- Event association (12B): failures are visible only in logs until 13D; a cluster that fails every run is retried every run.
